...
# ============================================================
# M15 DIVERGENCE WITH DEBUG
# ============================================================

def get_m15_divergence_with_debug(symbol: str, debug: bool = False) -> Dict[str, Any]:
    """
    Get M15 divergence with enhanced debugging.
    This is a debug version of the function from indicators.py.
    """
    try:
        ...
        rates_count = len(m15_rates) if m15_rates is not None else 0
        logger.info(f"[M15_DBG] {symbol}: rates={rates_count}")
        
        if m15_rates is None or rates_count < M15_MIN_BARS:
            logger.warning(f"[M15_DBG] {symbol}: Insufficient bars ({rates_count}/{M15_MIN_BARS})")
            return {
                "divergence_type": "NONE", 
                "divergence_score": 0, 
                "rsi_14": 50, 
                "timeframe": "M15",
                "debug": f"insufficient_data_{rates_count}"
            }
        
        # Extract price data
        close_prices = [float(bar[4]) for bar in m15_rates]
        
        # Calculate RSI (14 period)
        rsi_values = _calculate_rsi(close_prices, 14)
        
        if not rsi_values or len(rsi_values) < 50:
            logger.warning(f"[M15_DBG] {symbol}: RSI calculation failed (len={len(rsi_values)})")
            return {
                "divergence_type": "NONE", 
                "divergence_score": 0, 
                "rsi_14": 50, 
        ...
        # Detect divergence using local implementation
        divergence = _detect_rsi_divergence_local(close_prices, rsi_values, "M15")
        
        if debug:
            logger.info(f"[M15_DBG] {symbol}: divergence_type={divergence['type']}, score={divergence['score']}, rsi={current_rsi:.1f}")
        ...
        logger.error(f"[M15_DBG] {symbol}: Exception - {str(e)}")
        return {
            "divergence_type": "NONE", 
            "divergence_score": 0, 
            "rsi_14": 50, 
            "timeframe": "M15",
            "debug": f"exception_{str(e)[:50]}"
        }


def _detect_rsi_divergence_local(close_prices: List[float], rsi_values: List[float], timeframe: str) -> Dict[str, Any]:
    """
    Local implementation of RSI divergence detection for M15 debug.
    ...
    price_highs, price_lows = find_swings(close_prices, 3)
    rsi_highs, rsi_lows = find_swings(rsi_values, 3)
    ...
    if len(price_lows) >= 2 and len(rsi_lows) >= 2:
        ...
        if price_low_2 < price_low_1 and rsi_low_2 > rsi_low_1:
            price_diff_pct = abs((price_low_2 - price_low_1) / price_low_1) * 100
            rsi_diff = abs(rsi_low_2 - rsi_low_1)
            score = min(100, int(price_diff_pct * 10 + rsi_diff))
            return {"type": "REGULAR_BULLISH", "score": score}
    ...
    if len(price_highs) >= 2 and len(rsi_highs) >= 2:
        ...
        if price_high_2 > price_high_1 and rsi_high_2 < rsi_high_1:
            price_diff_pct = abs((price_high_2 - price_high_1) / price_high_1) * 100
            rsi_diff = abs(rsi_high_2 - rsi_high_1)
            score = min(100, int(price_diff_pct * 10 + rsi_diff))
            return {"type": "REGULAR_BEARISH", "score": -score}
    ...
    if len(price_lows) >= 2 and len(rsi_lows) >= 2:
        ...
        if price_low_2 > price_low_1 and rsi_low_2 < rsi_low_1:
            score = min(100, int(abs(rsi_low_2 - rsi_low_1) * 2))
            return {"type": "HIDDEN_BULLISH", "score": score * 0.7}
    ...
    if len(price_highs) >= 2 and len(rsi_highs) >= 2:
        ...
        if price_high_2 < price_high_1 and rsi_high_2 > rsi_high_1:
            score = min(100, int(abs(rsi_high_2 - rsi_high_1) * 2))
            return {"type": "HIDDEN_BEARISH", "score": -score * 0.7}
    
    return {"type": "NONE", "score": 0}


# ============================================================
# M15 DIVERGENCE FALLBACK (USES M1 DATA)
# ============================================================

def get_m15_divergence_fallback(symbol: str) -> Dict[str, Any]:
    """
    Fallback divergence detection using M1 data aggregated to M15.
    Use this if M15 data is unavailable or unreliable.
    """
    try:
        # Get M1 data and manually aggregate to M15 (15 minutes = 15 M1 bars)
        m1_rates = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_M1, 0, 900)  # 15 hours of M1
        
        if m1_rates is None or len(m1_rates) < 100:
            logger.warning(f"[M15_FALLBACK] {symbol}: Insufficient M1 data ({len(m1_rates) if m1_rates else 0})")
            return {"divergence_type": "NONE", "divergence_score": 0, "rsi_14": 50, "timeframe": "M15"}
        
        # Resample to M15 (every 15 M1 bars)
        m15_close = []
        
        for i in range(0, len(m1_rates) - 14, 15):
            window = m1_rates[i:i+15]
            if len(window) == 15:
                m15_close.append(float(window[-1][4]))  # Close of last M1 bar
        
        if len(m15_close) < 50:
            logger.warning(f"[M15_FALLBACK] {symbol}: Insufficient resampled bars ({len(m15_close)})")
            return {"divergence_type": "NONE", "divergence_score": 0, "rsi_14": 50, "timeframe": "M15"}
        
        # Calculate RSI on resampled data
        rsi_values = _calculate_rsi(m15_close, 14)
        
        if not rsi_values:
            return {"divergence_type": "NONE", "divergence_score": 0, "rsi_14": 50, "timeframe": "M15"}
        
        # Detect divergence
        divergence = _detect_rsi_divergence_local(m15_close, rsi_values, "M15")
        
        logger.info(f"[M15_FALLBACK] {symbol}: {divergence['type']} (score={divergence['score']}) using resampled M1->M15 data")
        ...
        logger.error(f"[M15_FALLBACK] {symbol}: Exception - {str(e)}")
        return {"divergence_type": "NONE", "divergence_score": 0, "rsi_14": 50, "timeframe": "M15"}
