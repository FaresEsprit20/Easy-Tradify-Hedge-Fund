...
# ✅ FIXED: Delegates to trade_persistence for consistent encoding
...
# ✅ NEW: analysis_at_close captured and saved to MongoDB
# ✅ FIXED: trade_id consistency (use "trade_" prefix)
...
# The direction a trade was OPENED in, looked up from the stored record rather
# than inferred from which way price moved.
from monitor.trade_persistence import _direction_of_record
...
class MultiSymbolMonitor:
    ...
    # How long the 2-hourly refresh waits for its analyses. It analyses every
    # filtered symbol (35 during US hours) on FILTER_THREADS threads at a few
    # seconds each; the old 20 s returned "0/12 chunks finished" at 14:24 on
    # 2026-09-17 and left the monitor with an empty watch list.
    ...
    def __init__(self):
        ...
        # _update_trade_price() is called once per position per
        ...
        #
        # Measured in the previous trade store before this fix: three trades open ~3.3h each
        ...
        print(f"✅ analysis_at_close: Captured and saved to MongoDB")
    ...
    # ============================================================
    # TRADE PERSISTENCE (MongoDB)
    # ============================================================

    def _save_trade_open(self, trade_result: TradeResult, analysis_result: Dict[str, Any]):
    ...
    # ============================================================
    # UPDATE TRADE PRICE EVOLUTION (MongoDB, encoded)
    # ============================================================

    def _update_trade_price(self, ticket: int, position: Dict[str, Any]):
        """
        Record one encoded price-evolution point for an open trade.
        Delegates to trade_persistence for consistent encoding.
        ...
        # Stop recording once the stored trade is closed. This used to ask a
        # store that holds no trades, so it never stopped.
        if is_trade_closed(ticket):
            print(f"⏭️ Trade {ticket} already closed - skipping price evolution update")
            return
        ...
        # ============================================================
        # DELEGATE TO trade_persistence FOR CONSISTENT ENCODING
        # ============================================================
        return update_trade_price(
            ticket=ticket,
            position=position,
            symbol_mt5_map=self.symbol_mt5_map,
            fixed_trade_size_usd=self.FIXED_TRADE_SIZE_USD,
    ...
    # ============================================================
    # SAVE ANALYSIS AT CLOSE (MongoDB)
    # ============================================================

    def _save_analysis_at_close(self, ticket: int, symbol: str,
                                order_type: str, close_analysis: Dict[str, Any]):
        """Attach the closing analysis to the stored trade.

        Delegates to trade_persistence rather than keeping its own copy. The
        copy that lived here bypassed MongoDB entirely, and also stored M5
        and H1 snapshots in breach of the M1-only storage rule the rest of the
        pipeline follows.
        """
        return save_analysis_at_close(
            ticket=ticket,
            symbol=symbol,
            order_type=order_type,
    ...
    # ============================================================
    # SAVE TRADE CLOSE (MongoDB, WITH PROFIT)
    # ============================================================

    def _save_trade_close(self, symbol: str, ticket: int, close_reason: str,
        ...
        """
        # Duplicate-close guard. It used to read a store that holds no
        # trades, so it could never fire and a webhook close and a fallback
        # detection could both write the same close.
        if is_trade_closed(ticket):
            print(f"⚠️ Trade {ticket} already closed - skipping duplicate save")
            return False
        ...
        if close_analysis is None:
            ...
            # monitor/trade_persistence.py, and this was a second copy of it:
        ...
        # Save analysis_at_close
        self._save_analysis_at_close(
            # Looked up, not inferred from the price move -- see
            # core/broker_facts.py and monitor/trade_persistence._direction_of_record.
            ticket, symbol,
    ...
    def _save_trailing_stop(self, ticket: int, symbol: str, action: str,
    ...
    def handle_trailing_webhook(self, data: Dict[str, Any]):
        """Handle a trailing-stop webhook; the move is recorded in MongoDB."""
    ...
    def _refresh_top_symbols(self):
        ...
        try:
            ...
            reported = {item.get("symbol") for item in all_results}

            with self._state_lock:
                previous = list(self.top_symbols)
                self.top_symbols = []
                ...
                for item in top:
                    ...
                    status = SymbolStatus(
                        ...
                        is_active=True,
                        exchange=self.SYMBOL_EXCHANGE_MAP.get(symbol, "FX"),
                ...
                # A symbol whose analysis did not come back in time keeps its
                # place: an unfinished refresh is not evidence that its
                # confidence fell. Before this, a timed-out refresh emptied the
                # watch list and nothing was checked for entry until the
                # 5-minute reanalysis re-added symbols one batch at a time.
                listed = {st.symbol for st in self.top_symbols}
                for st in previous:
                    if (st.symbol not in reported and st.symbol not in listed
                            and st.symbol in self.filtered_symbols):
                        self.top_symbols.append(st)
                        listed.add(st.symbol)
    ...
    def _check_entry(self, symbol: str) -> Dict[str, Any]:
        try:
            ...
            # The analysis publishes the floor its probability was judged
            # against (a calibrated model sets its own); the fixed 65 only
            # applies when it does not.
            _floor = (result.get("config") or {}).get("min_probability_for_entry")
            _threshold = _floor if isinstance(_floor, (int, float)) and (result.get("calibrated_decision") or {}).get("enter")                 else self.MIN_CONFIDENCE_THRESHOLD
            if should_enter and overall_confidence >= _threshold:
                ...
                reason = ""
                if not should_enter:
                    # ✅ NEW: Use explicit veto reason if available
                    veto_reason = entry_analysis.get("veto_reason")
                    reason = f"Entry conditions not met ({veto_reason})" if veto_reason else "Entry conditions not met"
    ...
    def _run_monitor_step(self):
        ...
        with self._state_lock:
            # Least recently checked first. In confidence order the same top
            # MONITOR_THREADS symbols were due again every step (a check takes
            # longer than SYMBOL_COOLDOWN), so every symbol below them was
            # never checked for entry at all.
            due = []
            for s in self.top_symbols:
                if (s.is_active and not s.in_position and
                    ...
                    if current_time - s.last_check_time >= self.SYMBOL_COOLDOWN:
                        due.append((s.last_check_time, s.symbol))
            symbols_to_monitor = [symbol for _, symbol in sorted(due)]
                    ...
                    if not found and confidence >= self.MIN_CONFIDENCE_THRESHOLD:
                        if len(self.top_symbols) < self.TOP_SYMBOLS_COUNT:
                            ...
                            if confidence > lowest.confidence:
                                ...
                                lowest.exchange = self.SYMBOL_EXCHANGE_MAP.get(symbol, "FX")
    ...
    def _execute_trade(self, symbol: str, analysis_result: Dict[str, Any]) -> TradeResult:
        ...
        try:
            ...
            execution_result = execute_trade(
                ...
                keep_stop=str(final_verdict.get("stop_source") or "").startswith("strategy_setup"),
            )
            ...
            if success and ticket:
                ...
                self._save_trade_open(trade_result, analysis_result)
            else:
                ...
                for ticket, symbol in list(pairs.items()):
                    try:
                        ...
                        if not position:
                            ...
                            # Already closed in the store? (prevents a duplicate close write)
                            already_closed = is_trade_closed(ticket)
                            if already_closed:
                                print(f"   ✅ Trade {ticket} already closed in MongoDB")
                            ...
                            # Double-check position is actually open at broker
                            is_open = get_position_details(ticket) is not None

                            if is_open:
                                self._update_trade_price(ticket, position)
                            else:
                                print(f"💤 Position {ticket} no longer open, skipping update")
    ...
    def _print_health_report(self):
        with self._state_lock:
            ...
            mongo_saves = self.stats.get("mongo_saves", 0)
            mongo_errors = self.stats.get("mongo_errors", 0)
        ...
        print(f"   💾 MongoDB Saves: {mongo_saves} (errors: {mongo_errors})")
    ...
    def start_non_blocking(self):
        ...
        print("✅ analysis_at_close: Captured and saved to MongoDB")
        print("✅ Trade id format: trade_{ticket} (MongoDB)")
